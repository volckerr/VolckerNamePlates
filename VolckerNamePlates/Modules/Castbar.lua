...
local function UpdateVisualState(f, spellName, texture, notInterruptible)
	...
	f.castbar.spellName = spellName

	if f.castbar.name and spellName then
		f.castbar.name:SetText(spellName)
	end
...
local function StopUnitCast(f)
	if not (f and f.castbar) then
		return
	end
...
local function StartUnitCast(f, unit, channel)
	if not (f and f.castbar and unit and UnitExists(unit)) then
		return
	end
	...
	f.castbar.startTime = startSeconds
	f.castbar.endTime = endSeconds
	f.castbar.channel = channel
	f.castbar.castID = castID

	UpdateVisualState(f, name, texture or (f.spell and f.spell:GetTexture()), notInterruptible)
	f.castbar.bar:SetMinMaxValues(0, duration)
	f.castbar.bar:SetValue(channel and duration or 0)

	f.castbar:SetScript(
		"OnUpdate",
		function(self)
			local now = GetTime()
			if not self.endTime or now >= self.endTime then
				StopUnitCast(f)
				return
			end
			...
			if self.curr then
				self.curr:SetText(format("%.1f", value))
			end

			if self.name and self.spellName then
	...
	if f.castbar.curr then
		f.castbar.curr:SetText(format("%.1f", channel and duration or 0))
	end

	f.castbar:Show()
end

local function UpdateUnitCast(unit, channel)
	local frame = GetFrameByUnit(unit)
	if frame then
		if channel == nil then
			local castName = UnitCastingInfo(unit)
			if castName then
				StartUnitCast(frame, unit, false)
				return
			end

			local channelName = UnitChannelInfo(unit)
			if channelName then
				StartUnitCast(frame, unit, true)
				return
			end

			StopUnitCast(frame)
		else
			StartUnitCast(frame, unit, channel)
		end
	end
end
...
function mod:CreateCastbar(frame)
	...
	frame.castbar.spark:SetTexture("Interface\\AddOns\\VolckerNamePlates\\Media\\t\\spark")
	...
	frame.castbar.shield:SetTexture("Interface\\AddOns\\VolckerNamePlates\\Media\\Shield")
