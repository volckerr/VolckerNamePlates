...
local category = "VolckerNamePlates"
...
do
	...
	local function AddGlobalConfigChanged(mod, target_module, key_groups, ro, pf)
		...
		if not target_module or target_module == "addon" then
			target_module = "VolckerNamePlates"
		end
	...
	local options = {
		...
		args = {
			...
			website = {
				type = "execute",
				name = L["Website"],
				func = function() StaticPopup_Show("VOLCKERNAMEPLATES_GITHUB") end,
				order = 2
			},
			general = {
				type = "group",
			...
			fade = {
				...
				args = {
					...
					targetalpha = {
						type = "range",
						name = L["Target alpha"],
						desc = L["The alpha value used by your current target's nameplate."],
						min = 0,
						max = 1,
						bigStep = .01,
						isPercent = true,
	...
	function addon:FinalizeOptions()
		...
		AceConfig:RegisterOptionsTable("volckernameplates", options)
		AceConfigDialog:AddToBlizOptions("volckernameplates", category)
...
--------------------------------------------------------------- Github repository --
StaticPopupDialogs.VOLCKERNAMEPLATES_GITHUB = {
...
--------------------------------------------------------------- Slash command --
_G.SLASH_VOLCKERNAMEPLATES1 = "/volckernameplates"
_G.SLASH_VOLCKERNAMEPLATES2 = "/vnp"

function SlashCmdList.VOLCKERNAMEPLATES()
	addon:OpenConfig()
end

function addon:OpenConfig()
	AceConfigDialog:SetDefaultSize("volckernameplates", 610, 500)
	AceConfigDialog:Open("volckernameplates")
end
function addon:CloseConfig()
	AceConfigDialog:Close("volckernameplates")
end
-- config handlers #############################################################
do
	...
	addon:AddConfigChanged(
		{"general", "yoffset"},
		function(val)
			addon.db.profile.general.yoffset = val
