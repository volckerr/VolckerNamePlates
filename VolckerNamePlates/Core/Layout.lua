...
local function UpdateTargetVisuals(self)
	if not self.friend then
		if self.target then
			self.name:SetTextColor(1, 1, 0, 1)
			self:SetGlowColour(1, 1, 1, 0.45)
			if self.targetGlow then
				self.targetGlow:SetVertexColor(1, 1, 1, 0.45)
			end
		else
			self.name:SetTextColor(1, 1, 1, 1)
			self:SetGlowColour()
			if self.targetGlow then
				self.targetGlow:SetVertexColor(unpack(profile.general.targetglowcolour))
			end
		end
	end
end

local function ApplyTargetState(self, isTarget)
	if isTarget then
		if self.target then
		...
		addon:SendMessage("VolckerNamePlates_PostTarget", self, true)
		UpdateTargetVisuals(self)
	elseif self.target then
		...
		addon:SendMessage("VolckerNamePlates_PostTarget", self, nil)
		UpdateTargetVisuals(self)
	end
end

local function GetTargetKuiFrame()
	...
	return addon.GetUnitPlate and addon:GetUnitPlate("target")
end

local function SyncTargetState()
	local targetFrame = GetTargetKuiFrame()

	for _, frame in pairs(addon.frameList) do
		if frame.kui then
			ApplyTargetState(frame.kui, frame.kui == targetFrame)
		end
	end
end
...
local function GetDesiredAlpha(frame)
	...
	if UnitExists("target") then
		return frame.defaultAlpha == 1 and profile_fade.targetalpha or profile_fade.fadedalpha
	else
...
local function OnFrameHide(self)
	...
	addon:SendMessage("VolckerNamePlates_PostHide", f)
end
...
local function UpdateFrame(self)
	...
	if self.DispatchPostShow then
		...
		addon:SendMessage("VolckerNamePlates_PostShow", self)
...
local function UpdateFrameCritical(self)
	...
	------------------------------------------------------------ Target stuff --
	-- target state is synced directly on PLAYER_TARGET_CHANGED and frame show
	-- to avoid relying on alpha-based target inference.
...
function addon:InitFrame(frame)
	...
	------------------------------------------------------------ Finishing up --
	addon:SendMessage("VolckerNamePlates_PostCreate", f)
