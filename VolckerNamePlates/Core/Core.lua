...
local addon = LibStub("AceAddon-3.0"):NewAddon("VolckerNamePlates", "AceEvent-3.0", "AceTimer-3.0")
addon.version = GetAddOnMetadata("VolckerNamePlates", "Version")
addon.website = "https://github.com/volckerr/VolckerNamePlates"
_G.VolckerNamePlates = addon
...
local NAMEPLATE_PLACEMENT_CVAR = "nameplatePlacement"
...
local defaults = {
	profile = {
		...
		fade = {
			...
			targetalpha = 1, -- the alpha value used by the current target
			rules = {
...
do
	...
	function addon:GetGUID(f)
		...
		if f.player and knownGUIDs[f.name.text] then
			...
			addon:SendMessage("VolckerNamePlates_GUIDAssumed", f)
		end
	end
	function addon:StoreGUID(f, unit, guid)
		...
		addon:SendMessage("VolckerNamePlates_GUIDStored", f, unit)
	end
	...
	local function StoreUnit(unit)
		...
		if f then
			...
			addon:SendMessage("VolckerNamePlates_UnitStored", unit, name, guid)
		end
	end
	...
	function addon:PLAYER_TARGET_CHANGED(event)
		...
		if addon.SyncTargetState then
			addon:SyncTargetState()
		end
	end
...
function addon:ApplyBaseNameplateOffset()
	if not SetCVar then
		return
	end

	local offset = (self.db and self.db.profile and self.db.profile.general and self.db.profile.general.yoffset or 0) / 100
	SetCVar(NAMEPLATE_PLACEMENT_CVAR, offset)
end
...
function addon:UpdateSizesTable()
	...
	addon.sizes.frame.yoffset = addon.db.profile.general.yoffset
...
function addon:OnInitialize()
	self.db = LibStub("AceDB-3.0"):New("VolckerNamePlatesGDB", defaults)
